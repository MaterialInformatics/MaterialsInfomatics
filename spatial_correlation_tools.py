...
def Correlations2D(D0s,s):
    ...
    for d0 in D0s:
        ...
        vard=var(d0)
        xp=a-mean
        corr=correlate_fft2D(xp,xp)/mean**2
        corrs.append(corr)
        print(d0)
    return corrs
...
def Plot2DCorrelations(D0s,corrs,labs,outpdir,s):
    for d0,corr,lab in zip(D0s,corrs,labs):
        ...
        s2=shape(corr)
        im2=ax_corr.imshow(corr[:int(3*s2[0]/4.),:int(3*s2[1]/4.)], cmap='copper',vmin=0)
...
def Construct3DCorrelationsFrom2D(corr):
    """
    Assume that zz correlations are equal to (xx + yy)/2.
    """
